in vec3 posWorldSpaceInterpolated;
in vec3 normalWorldSpaceInterpolated;
in vec3 tangentWorldSpaceInterpolated;
in vec3 binormWorldSpaceInterpolated;
...
void main() {
  ...
  vec3 N = normalize(normalWorldSpaceInterpolated);
  vec3 T = normalize(tangentWorldSpaceInterpolated);
  vec3 B = normalize(binormWorldSpaceInterpolated);
  ...
  vec3 lightVec = normalize(lightPosition - posWorldSpaceInterpolated);
  
  // ambient color
  vec3 ambient = ka * la;
  
  vec3 viewVec =  normalize(viewPos-posWorldSpaceInterpolated);
  float d = max(0.0, dot(N, lightVec));
  
  float s = 0.0;
  if(d > 0.0) {
    vec3 reflected =  reflect(-lightVec, N);
    s = pow(max(0.0, dot(viewVec, reflected)), shininess);
  }
  vec3 specular = s * ks * ls;
  
  // diffuse color
  vec3 reflectedView =  reflect(-viewVec, N); // reflect expects L pointing to surface
  vec3 diffuse = d*(kd*ld*0.5 + 0.5*texture(environment, reflectedView).xyz);
